...
def detect():
    ...
    try:
        ...
        if len(sensor_data) != INPUT_DIM:
            return jsonify({
                'error': f'Expected {INPUT_DIM} sensor values, got {len(sensor_data)}'
            }), 400
        ...
        attack_detected = error > THRESHOLD
        
        # CRITICAL: Convert NumPy/Torch types to Python types
        return jsonify({
            'attack_detected': bool(attack_detected),  # ← FIX HERE
            'reconstruction_error': float(error),       # ← FIX HERE
            'threshold': float(THRESHOLD),
            'status': 'ATTACK DETECTED' if attack_detected else 'NORMAL'
        })
